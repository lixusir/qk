...
class Product extends Model
{
    ...
    {   
        if(!empty($param['cate_id']) && $isedit){
            ...
            if($product['status']==1) delaggregate('productlist_cateid_'.$product['cate_id'],$product['id']);
    ...
    {
        $Product=self::where('id',$id)->find();
        $Product=$Product->toArray();
        self::where('id',$id)->delete();
        delaggregate('productlist_cateid_'.$Product['cate_id'],$id);
    }
