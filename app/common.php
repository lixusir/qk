...
function sendSms($mobile, $content, $type=0)
{
  	$SmsConf=Config::get('dayuconfig');
    $url='https://api.mysubmail.com/message/send.json';
  	$content="【{$SmsConf['default_sign']}】".preg_replace("/【.*】/","",$content);
  	//$content=substr_count($content,'【海洋世界】') != 0?str_replace('【海洋世界】',"【{$SmsConf['default_sign']}】",$content):"【{$SmsConf['default_sign']}】".preg_replace("/【.*】/","",$content);
   $postArr = array (

        'appid'  =>  $SmsConf['default_account'],

        'signature' => $SmsConf['default_password'],
...
file_put_contents(dirname(__FILE__).'/dd', json_encode($result));
...
function setMemberOnline($member)
{
    try{
        ...
        $res = $redis->ping('+PONG'); // 检测当前链接状态，返回PONG或者抛出异常。
        if($res === '+PONG'){
...
//删除集合中指定成员
function delaggregate($keyname,$id)
{
    $int=0;
    $data=array();
    $expireTime = getTodayEndtime();
    ...
    try{
        ...
        $res = $redis->ping('+PONG');
        if($res === '+PONG'){
           $member=$redis->scard($keyname);
          for($i=0;$i<$member;$i++){
          	$data[] = unserialize($redis->spop($keyname));
          }
          ...
          foreach ($data as $key => $value){
             $redis->sadd($keyname, serialize($value));
            $redis->expireAt($keyname, $expireTime); //过期时间
          }
